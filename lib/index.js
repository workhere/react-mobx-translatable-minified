 ...
 */
function _translatable(Component, connect) {
  ...
  Component.prototype.has = function (key, opts) {
    var includeDefault = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : false;
  ...
  var observed = connect ? mobxReact.observer(connect)(Component) : mobxReact.observer(Component);
  return mobxReact.inject(config.injectFn)(observed);
}

module.exports = {
  init: init,
  translatable: function translatable(arg) {
    if (arg instanceof Array) {
      return function (Component) {
        return _translatable(Component, arg);
      };
    }
    return _translatable(arg);
  }
};